...
public class SecurityController extends BaseController {
	...
	public HttpBaseDto saveSensitivePoint(@RequestParam String token, @RequestParam String name, @RequestParam String lat, @RequestParam String lng,
			@RequestParam Integer radius) {
		if (StringUtils.isAllEmpty(lat, lng)) {
			throw new BizException(RespCode.NOTEXIST_PARAM);
		}
		...
		Long user_id = checkTokenAndUser(token);
		if (this.sensitivePointService.insert(user_id, name, lat, lng, radius)) {
			HttpBaseDto dto = new HttpBaseDto();
			return dto;
		} else {
			logger.info(
	...
	public HttpBaseDto updateSensitivePoint(@RequestParam String token, @RequestParam Long id, @RequestParam String name, @RequestParam String lat,
			@RequestParam String lng, @RequestParam Integer radius) {
		if (StringUtils.isAllEmpty(lat, lng)) {
			throw new BizException(RespCode.NOTEXIST_PARAM);
		}
		...
		Long user_id = checkTokenAndUser(token);
		if (this.sensitivePointService.update(id, user_id, name, lat, lng, radius)) {
			HttpBaseDto dto = new HttpBaseDto();
			return dto;
		} else {
